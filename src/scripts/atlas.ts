...
document.querySelector("[data-clear-filters]")?.addEventListener("click", () => applyState({ view: resultRoot?.dataset.view || "cards" }));
...
window.addEventListener("popstate", () => applyState(parseFilterState(location.search), false));
